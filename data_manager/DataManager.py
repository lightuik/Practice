...
class DataManager:
    ...
    def search_data(self, keyword):  # 查找
        self.query = f"SELECT filename,binary_content FROM binary_code WHERE filename LIKE '{keyword}'"
        try:
            self.cursor.execute(self.query)
            outcome = self.cursor.fetchone()
            documents_list = [document[0] for document in outcome]
            binary_list = [document[1] for document in outcome]
            return dict(zip(documents_list, binary_list))
        except:
            return None
